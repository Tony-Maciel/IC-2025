!--------------------------------------------------------------------------------------------!
! Ising model simulator on square lattice, w/o external field, only NN, using the            !
! Wolff algorithm.                                                                           !
!                                                                                            ! 
...
!NOTE: compile with:
! gfortran -O3 -march=native -flto rng_wrapper.o Wolff.f90 -L. -lRANLUX++ -lranluxpp_c -Wl,-rpath='$ORIGIN' -o Wolff
...
contains 
  ...
  subroutine init_sys(self, L, dim, p, rng)
    ...
    type(rng_t), intent(in out)   :: rng ! asssumes its already initialized.
    integer                       :: i
  ...
  subroutine MCstep(self, rng, P_add)  
    class(System), intent(in out) :: self 
    type(rng_t), intent(in out)   :: rng 
    real(dp), intent(in)          :: P_add
    integer                       :: site, i, idx_cl, idx_st, neighb

    ! assumes self%in_cluster(:) = .false.
    idx_cl                = 1 !index for cluster
    idx_st                = 1 ! index for stack
    site                  = int(next_rng(rng) * self%Total) + 1   ! randomly choose spin
    ...
    do while(idx_st .gt. 1) ! while still have spins in stack...
      ! pop spin from stack 
      site   = self%stack(idx_st-1)
      idx_st = idx_st - 1
      do i = 1, 2*self%dim ! for each of its neighbors
        neighb = self%Neighbors(site,i)
        if (self%in_cluster(neighb)) cycle ! already in cluster
        if (self%Status(neighb) * self%Status(site) .gt. 0.0_dp) then ! same spin 
          if (next_rng(rng) .lt. P_add) then ! add to cluster and stack
    ...
    ! Flip all spins in cluster and reset it
    idx_cl = idx_cl - 1
    do i = 1, idx_cl
      site                  = self%cluster(i)
      self%Status(site)     = -self%Status(site)
      self%in_cluster(site) = .false.
    end do
    self%cluster_size = self%cluster_size + idx_cl ! for average later
  ...
  subroutine Get(sys, meas, mcs_idx) 
    ...
    do site = 1, sys%Total 
      ...
      do direction = 1, sys%dim ! sums across only half the neighbors to avoid repeated counting
        ...
        do r = 2, (sys%L/2 - 1) ! for all distances r between sites...
    ...
    meas%tot_E = E_temp     ! total energy and magnetization for multiple histogram method
    meas%tot_M = M_temp**2

    meas%E1 = E_temp / sys%Total 
    meas%E2 = meas%E1 * meas%E1  
    meas%M1 = (M_temp / sys%Total)**2 ! square to not get <m> = 0.0 for T < Tc (only for Wolff)
  ...
  subroutine Run(sys, meas, beta, rng, idx, int_param)
    ...
    real(dp)                           :: samples, mean_mt, mean_mtp, Et, Mt, Ct, Xt, P_add  ! temporary variables
    real(dp)                           :: re_C1, re_C2, re_X1, re_X2, C1, C2, X1, X2, rE, rm ! bootstrapping
    real(dp)                           :: bc 
    integer                            :: i, j, k, io
    character(len=100)                 :: mcresults

    P_add            = 1.0_dp - exp(-2.0_dp*beta)
    Et               = 0.0_dp 
    Mt               = 0.0_dp 
    Ct               = 0.0_dp 
    Xt               = 0.0_dp
    ...
    Mt              = Mt / real(meas%N,dp) ! <m^2> (remember: order parameter is m^2)
    Ct              = Ct / real(meas%N,dp) ! <E^2> 
    Xt              = Xt / real(meas%N,dp) ! <m^4>
    bc              = bc / real(meas%N,dp) ! <m^8>
    meas%tcorr(0)   = Xt - Mt*Mt ! C(t=0) = <m^4> - <m^2>^2, temporal correlation at t=0
    meas%corr(:)    = meas%corr(:) / (real(sys%dim * sys%Total,dp) * real(meas%N,dp)) - Mt
    meas%E(idx)     = Et 
    meas%M(idx)     = Mt 
    ...
    ! total number of sites per average cluster
    sys%cluster_size = sys%cluster_size / real(meas%N * meas%lag, dp)
...
program main
  ...
  do k = 1, n_sizes
    ...
    write(output,'(A,I0,A,I0,A,I0,A)') "W_EMCX_", L, "_", dim, "-", int_param, ".dat"  ! prefiix W_ for Wolff
    ...
    write(corr_output,'(A,I0,A,I0,A,I0,A)') "W_CORR_", L, "_", dim, "-", int_param, ".dat" 
    ...
    write(tcorr_output,'(A,I0,A,I0,A,I0,A)') "W_TCORR_", L, "_", dim, "-", int_param, ".dat" 
    ...
    do i = 1, vals%length 
      ...
      open(newunit=io, file=tcorr_output, action="write", position="append")
        write(io,*) current_temp, sample%cluster_size, vals%tcorr(0:TNC-1)  ! Not divided by C(t=0)
      close(io)
