...
contains 
  ...
  ! ONE MONTE CARLO STEP USING THE METROPOLIS ALGORITHM
  subroutine MCstep(self, rng)  
    class(System), intent(in out) :: self 
    type(rng_t), intent(in out)   :: rng 
    integer                       :: site, dU, i 

    ! For one MC step (metropolis)
    do i = 1, self%Total
      site = 1 + int(next_rng(rng) * self%Total)
      dU   = int(sum(self%Status(self%Neighbors(site,:))) * 2 * self%status(site))
      if (dU .le. 0 .or. next_rng(rng) .lt. self%expdus(dU)) self%Status(site) = -self%Status(site)
    end do
  ...
  subroutine Run(sys, meas, beta, rng, idx, int_param)
    ...
    ! set up exponential vector for Metropolis algorithm
    do i = 1 , (2*sys%dim + 1)
      dutemp = 4*(sys%dim - i + 1)
      sys%expdus(dutemp) = exp(-real(dutemp,dp) * beta)
    end do
    ...
    ! meas%E_err(idx) = error(meas%mc_E, Et, meas%N)  ! If only one node is used, uncomment this...
    ! meas%M_err(idx) = error(meas%mc_M, Mt, meas%N)  ! 
    meas%E_err(idx) = 0.0_dp                          ! ...and comment this
...
program main
  ...
  open(newunit=io, file="Ising_input.in", action="read")
    ...
    read(io,*) n_sizes! total number of system sizes to use (e.g., 1->32, 2->32,64, ...)
    ...
    read(io,*) nboot  ! number of resamples for bootstrapping.
  ...
  do k = 1, n_sizes
    ...
    call sample%set_neighbors(1)                               ! needs argument to be 1 to start recursion
    ...
    do i = 1, vals%length 
      ...
      open(newunit=io, file=tcorr_output, action="write", position="append")
        ! sample%cluster_size == 0.0_dp to avoid any more modifications to show.py (no clusters in metropolis)
